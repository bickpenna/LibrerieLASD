...
// 3) Lascia solo arco con peso maggiore
void eliminaPesiMinori(link *head, link *next){
    if(head == NULL ||  next == NULL) return;
    if(*head == NULL ||  *next == NULL) return;

    if((*head)->peso >= (*next)->peso){
        (*head)->next = (*next)->next;
        eliminaPesiMinori(head, &(*head)->next);
    } else{
        elTesta(head);
        eliminaPesiMinori(next, &(*next)->next);
    }
}

void lasciaSoloArcoMaggiore(Grafo graph){
    int src;
    for(src = 0; src < graph.dim; src++){
        eliminaPesiMinori(&graph.arr[src].adj, &graph.arr[src].adj->next);
    }
}
...
int main(){
    ...
    creaArcoGAL(graphL1, 0, 2, 3);
    creaArcoGAL(graphL1, 1, 2, 5);
    creaArcoGAL(graphL1, 1, 3, 9);
    creaArcoGAL(graphL1, 1, 4, 3);
    creaArcoGAL(graphL1, 2, 3, 5);
    creaArcoGAL(graphL1, 2, 4, 7);
    ...
    //Lascia solo arco con peso maggiore
    lasciaSoloArcoMaggiore(graphL1);

    printf("\nGrafo 1 con solo arco maggiore: \n");
    stampaGrafoAL(graphL1);
}